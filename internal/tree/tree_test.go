...
func TestBuildAndVisible(t *testing.T) {
	...
	got = names(tr.Visible("", StatusNone))
	want = "Sample| Alpha.Tests|  Alpha.Other.X|   Y|  MathTests|   Adds|   Theory|  SlowTests|   Waits"
	if got != want {
	...
	if got != "Sample| Alpha.Tests|  SlowTests|   Waits" {
...
func TestVisibleByStatus(t *testing.T) {
	...
	if got := names(tr.Visible("", StatusFailed)); got != "Sample| Alpha.Tests|  MathTests|   Theory|    (n: 2)" {
		t.Errorf("failed only = %s", got)
	}
	if got := names(tr.Visible("", StatusSkipped)); got != "Sample| Alpha.Tests|  SlowTests|   Waits" {
...
func TestFoldByResult(t *testing.T) {
	...
	tr.FoldByResult(p)
	got := names(tr.Visible("", StatusNone))
	want := "Sample| Alpha.Tests|  Alpha.Other.X|  MathTests|   Adds|   Theory|    (n: 1)|    (n: 2)|  SlowTests"
	if got != want {
		t.Fatalf("folded =\n%s\nwant\n%s", got, want)
	}
}
